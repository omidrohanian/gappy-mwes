...
class Main(object):
    ...
    def run(self):
        ...
        ################### Added by SHIVA
        gap2stats = collections.defaultdict(lambda: Statistics(mc_args))
        ###################
...
################################################################################### Added by SHIVA
            for gap_size in range(10):
                g = self.mweinfo_per_gap(mweinfos_gold, gap_size)
                p = self.mweinfo_per_gap(mweinfos_pred, gap_size)
                self.add_to_stats(sent_gold, gap2stats[gap_size], g, p,
                    debug_header="Gap_Size: {}".format(str(gap_size)) )
...
###################################################################### Added by SHIVA
        print("## MWE with different gap sizes (partition of Global)")
        for gap_size in range(10):
            prefix = "Gap Size {}: ".format(str(gap_size))
            gap2stats[gap_size].print_mwebased_proportion(
                        prefix, baseline=base_stats)
            gap2stats[gap_size].c_mwebased.print_p_r_f(prefix)
        print()
######################################################################
...
################################################################################### Added by SHIVA
    def mweinfo_per_gap(self, mweinfos: list, gap_size: list):
        r'''Return a sublist of MWEInfo instances with different gap size.'''
        return [m for m in mweinfos if gap_size == m.n_gaps()]
###################################################################################
